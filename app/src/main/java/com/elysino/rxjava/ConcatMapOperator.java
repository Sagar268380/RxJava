...
Now consider the same example of FlatMap but replacing the operator with ConcatMap.
 Technically the both operators produces the same output but the sequence the data emitted changes.
...
public class ConcatMapOperator extends AppCompatActivity {
    private static final String TAG = ConcatMapOperator.class.getSimpleName();
